...
与其他分布式系统一样，MinIO 在同一 :term:`server pool` 内的所有节点都使用相同配置时，通常能获得更好的效果。
...
.. tab-set::
   ...
   .. tab-item:: Kubernetes
      ...
      请确保每个 Tenant :ref:`server pool <minio-intro-server-pool>` 内所有节点呈现给各 PV 的容量一致。
...
请在每个 MinIO :ref:`server pool <minio-intro-server-pool>` 的所有节点上使用相同容量和类型的驱动器。
...
.. important::
   ...
   本节提到的工具 **需要** MinIO 订阅。
   MinIO 强烈建议所有生产部署结合其 SUBNET 许可证使用 `AIStor Object Store <https://minio.pigsty.io/index.html>`__。
